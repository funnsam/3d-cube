...
pub fn render(state: &mut State, size: usize) -> Vec<Vec<bool>> {
    struct ProjectedPoint { x: f32, y: f32 }
    struct ScreenPoint { x: isize, y: isize }

    let mut screen = vec![vec![false; size]; size];
    ...
    for v in state.v.iter() {
        ...
        verts.push(ProjectedPoint {
            x:   (x * state.f) / (z + state.f),
            y: -((y * state.f) / (z + state.f))
        });
    }
...
fn plot(screen: &mut Vec<Vec<bool>>, size: usize, x: isize, y: isize) {
